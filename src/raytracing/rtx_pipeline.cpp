...
#include "nvpro_core/nvh/alignment.h"
#include "nvpro_core/nvh/fileoperations.h"
#include "nvpro_core/nvvk/shaders_vk.h"
...
//
void RtxPipeline::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t familyIndex, bgfx::ResourceAllocator* allocator)
{
  m_device     = device;
  m_pAlloc     = allocator;
  ...
  BGFX_VKAPI(vkGetPhysicalDeviceProperties2)(physicalDevice, &properties);
...
void RtxPipeline::destroy()
{
  ...
  BGFX_VKAPI(vkDestroyPipeline)(m_device, m_rtPipeline, nullptr);
  BGFX_VKAPI(vkDestroyPipelineLayout)(m_device, m_rtPipelineLayout, nullptr);
...
void RtxPipeline::createPipeline()
{
  ...
  // Raygen
  stage.module    = bgfx::createShaderModule(m_device, pathtrace_rgen, sizeof(pathtrace_rgen));
  stage.stage     = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
  stages[eRaygen] = stage;

  // Miss
  stage.module  = bgfx::createShaderModule(m_device, pathtrace_rmiss, sizeof(pathtrace_rmiss));
  stage.stage   = VK_SHADER_STAGE_MISS_BIT_KHR;
  stages[eMiss] = stage;

  // The second miss shader is invoked when a shadow ray misses the geometry. It simply indicates that no occlusion has been found
  stage.module   = bgfx::createShaderModule(m_device, pathtraceShadow_rmiss, sizeof(pathtraceShadow_rmiss));
  stage.stage    = VK_SHADER_STAGE_MISS_BIT_KHR;
  stages[eMiss2] = stage;

  // Hit Group - Closest Hit
  stage.module        = bgfx::createShaderModule(m_device, pathtrace_rchit, sizeof(pathtrace_rchit));
  stage.stage         = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
  stages[eClosestHit] = stage;

  // Hit Group - Any Hit
  stage.module    = bgfx::createShaderModule(m_device, pathtrace_rahit, sizeof(pathtrace_rahit));
  ...
  if(useDeferred)
  {
    result = BGFX_VKAPI(vkCreateDeferredOperationKHR)(m_device, nullptr, &deferredOp);
    assert(result == VK_SUCCESS);
  }

  BGFX_VKAPI(vkCreateRayTracingPipelinesKHR)(m_device, deferredOp, {}, 1, &rayPipelineInfo, nullptr, &m_rtPipeline);

  if(useDeferred)
  {
    // Query the maximum amount of concurrency and clamp to the desired maximum
    uint32_t maxThreads{8};
    uint32_t numLaunches = std::min(BGFX_VKAPI(vkGetDeferredOperationMaxConcurrencyKHR)(m_device, deferredOp), maxThreads);
    ...
    for(uint32_t i = 0; i < numLaunches; i++)
    {
      ...
      joins.emplace_back(std::async(std::launch::async, [device, deferredOp]() {
        ...
        BGFX_VKAPI(vkDeferredOperationJoinKHR)(device, deferredOp);
      }));
    }
    ...
    result = BGFX_VKAPI(vkGetDeferredOperationResultKHR)(m_device, deferredOp);
    assert(result == VK_SUCCESS);
	BGFX_VKAPI(vkDestroyDeferredOperationKHR)(m_device, deferredOp, nullptr);
  }
...
//
void RtxPipeline::run(const VkCommandBuffer& cmdBuf, const VkExtent2D& size, bgfx::ProfilerVK& profiler, const std::vector<VkDescriptorSet>& descSets)
{
  LABEL_SCOPE_VK(cmdBuf);

  BGFX_VKAPI(vkCmdBindPipeline)(cmdBuf, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, m_rtPipeline);
  ...
  BGFX_VKAPI(vkCmdTraceRaysKHR)(cmdBuf, &regions[0], &regions[1], &regions[2], &regions[3], size.width, size.height, 1);
}
