...
#include "nvpro_core/nvvk/pipeline_vk.h"
#include "nvpro_core/nvvk/renderpasses_vk.h"
...
namespace bgfx{
	...
	void RenderOutput::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t familyIndex, bgfx::ResourceAllocator* allocator)
	{
		m_device = device;
		m_pAlloc = allocator;
		...
		m_offscreenDepthFormat = bgfx::findDepthFormat(physicalDevice);
	}
	...
	void RenderOutput::destroy()
	{
		...
		BGFX_VKAPI(vkDestroyPipeline)(m_device, m_postPipeline, nullptr);
		BGFX_VKAPI(vkDestroyPipelineLayout)(m_device, m_postPipelineLayout, nullptr);
		BGFX_VKAPI(vkDestroyDescriptorPool)(m_device, m_postDescPool, nullptr);
		BGFX_VKAPI(vkDestroyDescriptorSetLayout)(m_device, m_postDescSetLayout, nullptr);
	}
	...
	void RenderOutput::createOffscreenRender(const VkExtent2D& size)
	{
		...
		{
			auto colorCreateInfo = bgfx::makeImage2DCreateInfo(
			...
			bgfx::Image image = m_pAlloc->createImage(colorCreateInfo);
			NAME_VK(image.image);
			VkImageViewCreateInfo ivInfo = bgfx::makeImageViewCreateInfo(image.image, colorCreateInfo);
		...
		// Setting the image layout for both color and depth
		{
			...
			bgfx::cmdBarrierImageLayout(cmdBuf, m_offscreenColor.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
	...
	void RenderOutput::createPostDescriptor()
	{
		...
		m_postDescSet = bgfx::allocateDescriptorSet(m_device, m_postDescPool, m_postDescSetLayout);
	...
	void RenderOutput::run(VkCommandBuffer cmdBuf)
	{
		...
		BGFX_VKAPI(vkCmdPushConstants)(cmdBuf, m_postPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(Tonemapper), &m_tonemapper);
		BGFX_VKAPI(vkCmdBindPipeline)(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, m_postPipeline);
		BGFX_VKAPI(vkCmdBindDescriptorSets)(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, m_postPipelineLayout, 0, 1, &m_postDescSet, 0, nullptr);
	...
	{
		LABEL_SCOPE_VK(cmdBuf);
		bgfx::cmdGenerateMipmaps(cmdBuf, m_offscreenColor.image, m_offscreenColorFormat, m_size, bgfx::mipLevels(m_size), 1,
