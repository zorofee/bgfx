...
namespace bgfx {
	...
	void HdrSampling::setup(const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t familyIndex, ResourceAllocator* allocator)
	{
		m_device = device;
		m_alloc = allocator;
	...
	void HdrSampling::loadEnvironment(const std::string& hrdImage)
	{
		...
		VkImageCreateInfo icInfo = makeImage2DCreateInfo(imgSize, format);

		{
			// We are using a different family index (1 - transfer), to allow loading in a different
			// queue/thread that the display (0)
			VkQueue queue;
			BGFX_VKAPI(vkGetDeviceQueue)(m_device, m_familyIndex, 0, &queue);

			ScopeCommandBuffer cmdBuf(m_device, m_familyIndex, queue);
			Image              image = m_alloc->createImage(cmdBuf, bufferSize, pixels, icInfo);
			VkImageViewCreateInfo    ivInfo = makeImageViewCreateInfo(image.image, icInfo);
