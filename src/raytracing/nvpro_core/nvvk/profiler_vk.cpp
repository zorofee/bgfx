...
void ProfilerVK::deinit()
{
  if(m_queryPool)
  {
    BGFX_VKAPI(vkDestroyQueryPool)(m_device, m_queryPool, nullptr);
...
void ProfilerVK::resize()
{
  ...
  VkResult res = BGFX_VKAPI(vkCreateQueryPool)(m_device, &createInfo, nullptr, &m_queryPool);
  assert(res == VK_SUCCESS);

  bgfx::DebugUtil(m_device).setObjectName(m_queryPool, m_debugName);
}
...
bool ProfilerVK::getSectionTime(SectionID i, uint32_t queryFrame, double& gpuTime)
{
  ...
  uint64_t times[2];
  VkResult result = BGFX_VKAPI(vkGetQueryPoolResults)(m_device, m_queryPool, idxBegin, 2, sizeof(uint64_t) * 2, times, sizeof(uint64_t),
