...
bool DedicatedMemoryAllocator::init(VkDevice device, VkPhysicalDevice physDevice)
{
  ...
  BGFX_VKAPI(vkGetPhysicalDeviceMemoryProperties)(m_physicalDevice, &m_physicalMemoryProperties);
...
MemHandle DedicatedMemoryAllocator::allocMemory(const MemAllocateInfo& allocInfo, VkResult *pResult)
{
  ...
  VkResult result = BGFX_VKAPI(vkAllocateMemory)(m_device, &bakedInfo.memAllocInfo, nullptr, &memory);
  ...
  if(!allocInfo.getDebugName().empty())
  {
    ...
    bgfx::DebugUtil(m_device).setObjectName(memInfo.memory, localInfo.getDebugName());
  }
...
void DedicatedMemoryAllocator::freeMemory(MemHandle memHandle)
{
  ...
  BGFX_VKAPI(vkFreeMemory)(m_device, dedicatedHandle->getMemory(), nullptr);
...
void* DedicatedMemoryAllocator::map(MemHandle memHandle, VkDeviceSize offset, VkDeviceSize size, VkResult *pResult)
{
  auto  dedicatedHandle = castDedicatedMemoryHandle(memHandle);
  void* ptr             = nullptr;
  VkResult result = BGFX_VKAPI(vkMapMemory)(m_device, dedicatedHandle->getMemory(), offset, size, 0 /*VkMemoryFlags*/, &ptr);
...
{
  auto dedicatedHandle = castDedicatedMemoryHandle(memHandle);
  BGFX_VKAPI(vkUnmapMemory)(m_device, dedicatedHandle->getMemory());
}
