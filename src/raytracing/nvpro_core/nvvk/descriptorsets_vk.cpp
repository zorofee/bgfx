                                                        ...
                                                        VkPipelineLayoutCreateFlags flags /*= 0*/)
{
  ...
  result = BGFX_VKAPI(vkCreatePipelineLayout)(m_device, &layoutCreateInfo, nullptr, &m_pipelineLayout);
...
void DescriptorSetContainer::deinitPool()
{
  ...
  if(m_pool)
  {
	 BGFX_VKAPI(vkDestroyDescriptorPool)(m_device, m_pool, nullptr);
...
void DescriptorSetContainer::deinitLayout()
{
  if(m_pipelineLayout)
  {
	BGFX_VKAPI(vkDestroyPipelineLayout)(m_device, m_pipelineLayout, nullptr);
    m_pipelineLayout = VK_NULL_HANDLE;
  }

  if(m_layout)
  {
	BGFX_VKAPI(vkDestroyDescriptorSetLayout)(m_device, m_layout, nullptr);
...
VkDescriptorSetLayout DescriptorSetBindings::createLayout(VkDevice device, VkDescriptorSetLayoutCreateFlags flags, DescriptorSupport supportFlags) const
{
  ...
  result = BGFX_VKAPI(vkCreateDescriptorSetLayout)(device, &createInfo, nullptr, &descriptorSetLayout);
  assert(result == VK_SUCCESS);
...
VkDescriptorPool DescriptorSetBindings::createPool(VkDevice device, uint32_t maxSets /*= 1*/, VkDescriptorPoolCreateFlags flags /*= 0*/) const
{
  ...
  // scene pool
  result = BGFX_VKAPI(vkCreateDescriptorPool)(device, &descrPoolInfo, nullptr, &descrPool);
  assert(result == VK_SUCCESS);
  return descrPool;
}
