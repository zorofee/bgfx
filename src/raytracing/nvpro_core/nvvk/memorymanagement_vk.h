...
public:
  ...
  DeviceMemoryAllocator() { m_debugName = "bgfx::DeviceMemoryAllocator:" + std::to_string((uint64_t)this); }
...
protected:
  ...
  {
    //s_allocDebugBias++;
    return BGFX_VKAPI(vkAllocateMemory)(m_device, &memInfo, nullptr, &deviceMemory);
  }
  virtual void freeBlockMemory(BlockID id, VkDeviceMemory deviceMemory)
  {
    //s_allocDebugBias--;
	BGFX_VKAPI(vkFreeMemory)(m_device, deviceMemory, nullptr);
  }
  ...
  {
    return BGFX_VKAPI(vkCreateBuffer)(device, info, nullptr, buffer);
  }

  virtual VkResult createImageInternal(VkDevice device, const VkImageCreateInfo* info, VkImage* image)
  {
    return BGFX_VKAPI(vkCreateImage)(device, info, nullptr, image);
  }
};
