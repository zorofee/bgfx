...
namespace bgfx {
	...
	void cmdBegin(VkCommandBuffer cmd, VkCommandBufferUsageFlags flags)
	{
		...
		VkResult res = BGFX_VKAPI(vkBeginCommandBuffer)(cmd, &beginInfo);
	...
	void CommandPool::init(VkDevice device, uint32_t familyIndex, VkCommandPoolCreateFlags flags, VkQueue defaultQueue)
	{
		...
		BGFX_VKAPI(vkCreateCommandPool)(m_device, &info, nullptr, &m_commandPool);
		if (defaultQueue)
		{
	...
	void CommandPool::deinit()
	{
		if (m_commandPool)
		{
			BGFX_VKAPI(vkDestroyCommandPool)(m_device, m_commandPool, nullptr);
		...
		const VkCommandBufferInheritanceInfo* pInheritanceInfo /*= nullptr*/)
	{
		...
		VkCommandBuffer cmd;
		BGFX_VKAPI(vkAllocateCommandBuffers)(m_device, &allocInfo, &cmd);
		...
		if (begin)
		{
			...
			BGFX_VKAPI(vkBeginCommandBuffer)(cmd, &beginInfo);
		}
	...
	{
		BGFX_VKAPI(vkFreeCommandBuffers)(m_device, m_commandPool, (uint32_t)count, cmds);
	}
	...
	void CommandPool::submitAndWait(size_t count, const VkCommandBuffer* cmds, VkQueue queue)
	{
		...
		VkResult result = BGFX_VKAPI(vkQueueWaitIdle)(queue);
		if (bgfx::checkResult(result, __FILE__, __LINE__))
		{
			exit(-1);
		}
		BGFX_VKAPI(vkFreeCommandBuffers)(m_device, m_commandPool, (uint32_t)count, cmds);
	...
	void CommandPool::submit(size_t count, const VkCommandBuffer* cmds, VkQueue queue, VkFence fence)
	{
		...
		{
			BGFX_VKAPI(vkEndCommandBuffer)(cmds[i]);
		}

		VkSubmitInfo submit = { VK_STRUCTURE_TYPE_SUBMIT_INFO };
		submit.pCommandBuffers = cmds;
		submit.commandBufferCount = (uint32_t)count;
		BGFX_VKAPI(vkQueueSubmit)(queue, 1, &submit, fence);
	}
	...
	void RingFences::init(VkDevice device, uint32_t ringSize)
	{
		...
		for (uint32_t i = 0; i < m_cycleSize; i++)
		{
			...
			info.flags = 0;
			VkResult result = BGFX_VKAPI(vkCreateFence)(device, &info, nullptr, &m_fences[i].fence);
	...
	void RingFences::deinit()
	{
		...
		for (uint32_t i = 0; i < m_cycleSize; i++)
		{
			BGFX_VKAPI(vkDestroyFence)(m_device, m_fences[i].fence, nullptr);
		}
		m_fences.clear();
		m_device = VK_NULL_HANDLE;
	}
	...
	void RingFences::setCycleAndWait(uint32_t cycle)
	{
		...
		if (entry.active)
		{
			...
			VkResult result = BGFX_VKAPI(vkWaitForFences)(m_device, 1, &entry.fence, VK_TRUE, ~0ULL);
			if (bgfx::checkResult(result, __FILE__, __LINE__))
			{
				exit(-1);
			}
		...
		BGFX_VKAPI(vkResetFences)(m_device, 1, &entry.fence);
	}
	...
	void RingCommandPool::init(VkDevice device, uint32_t queueFamilyIndex, VkCommandPoolCreateFlags flags, uint32_t ringSize)
	{
		...
		for (uint32_t i = 0; i < m_cycleSize; i++)
		{
			...
			VkResult result = BGFX_VKAPI(vkCreateCommandPool)(m_device, &info, nullptr, &m_pools[i].pool);
		}
	}
	...
	void RingCommandPool::deinit()
	{
		...
		for (uint32_t i = 0; i < m_cycleSize; i++)
		{
			...
			if (!entry.cmds.empty())
			{
				BGFX_VKAPI(vkFreeCommandBuffers)(m_device, entry.pool, uint32_t(entry.cmds.size()), entry.cmds.data());
				BGFX_VKAPI(vkResetCommandPool)(m_device, entry.pool, VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT);
				entry.cmds.clear();
			}
			BGFX_VKAPI(vkDestroyCommandPool)(m_device, entry.pool, nullptr);
		}
	...
	void RingCommandPool::setCycle(uint32_t cycle)
	{
		...
		{
			BGFX_VKAPI(vkFreeCommandBuffers)(m_device, entry.pool, uint32_t(entry.cmds.size()), entry.cmds.data());
			BGFX_VKAPI(vkResetCommandPool)(m_device, entry.pool, 0);
			entry.cmds.clear();
		}
	}
		...
		const VkCommandBufferInheritanceInfo* pInheritanceInfo /*= nullptr*/)
	{
		...
		VkCommandBuffer cmd;
		BGFX_VKAPI(vkAllocateCommandBuffers)(m_device, &info, &cmd);
	...
	const VkCommandBuffer* RingCommandPool::createCommandBuffers(VkCommandBufferLevel level, uint32_t count)
	{
		...
		BGFX_VKAPI(vkAllocateCommandBuffers)(m_device, &info, cmds);
	...
	VkResult BatchSubmission::execute(VkFence fence /*= nullptr*/, uint32_t deviceMask)
	{
		...
		if (m_queue && (fence || !m_commands.empty() || !m_signals.empty() || !m_waits.empty()))
		{
			...
			res = BGFX_VKAPI(vkQueueSubmit)(m_queue, 1, &submitInfo, fence);
	...
	{
		VkResult result = BGFX_VKAPI(vkQueueWaitIdle)(m_queue);
		if (bgfx::checkResult(result, __FILE__, __LINE__))
		{
			exit(-1);
		}
