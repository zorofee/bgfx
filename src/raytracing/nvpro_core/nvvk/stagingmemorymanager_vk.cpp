                                    ...
                                    VkImageLayout                   layout)
{
  ...
  BGFX_VKAPI(vkCmdCopyBufferToImage)(cmd, srcBuffer, image, layout, 1, &cpy);
...
void* StagingMemoryManager::cmdToBuffer(VkCommandBuffer cmd, VkBuffer buffer, VkDeviceSize offset, VkDeviceSize size, const void* data)
{
  ...
  BGFX_VKAPI(vkCmdCopyBuffer)(cmd, srcBuffer, buffer, 1, &cpy);
...
const void* StagingMemoryManager::cmdFromBuffer(VkCommandBuffer cmd, VkBuffer buffer, VkDeviceSize offset, VkDeviceSize size)
{
  ...
  BGFX_VKAPI(vkCmdCopyBuffer)(cmd, buffer, dstBuffer, 1, &cpy);
                                            ...
                                            VkImageLayout                   layout)
{
  ...
  BGFX_VKAPI(vkCmdCopyImageToBuffer)(cmd, image, layout, dstBuffer, 1, &cpy);
...
StagingMemoryManager::SetID StagingMemoryManager::finalizeResourceSet()
{
  ...
  m_sets[m_stagingIndex].fence     = NULL;
...
void StagingMemoryManager::releaseResources()
{
  ...
  {
    if(!itset.entries.empty() && !itset.manualSet && (!itset.fence || BGFX_VKAPI(vkGetFenceStatus)(m_device, itset.fence) == VK_SUCCESS))
    {
