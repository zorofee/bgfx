...
void SamplerPool::deinit()
{
  ...
  for(auto it : m_entries)
  {
    if(it.sampler)
    {
		BGFX_VKAPI(vkDestroySampler)(m_device, it.sampler, nullptr);
    }
  }
...
VkSampler SamplerPool::acquireSampler(const VkSamplerCreateInfo& createInfo)
{
  ...
  if(it == m_stateMap.end())
  {
    ...
    VkSampler sampler;
    VkResult  result = BGFX_VKAPI(vkCreateSampler)(m_device, &createInfo, nullptr, &sampler);
...
void SamplerPool::releaseSampler(VkSampler sampler)
{
  ...
  if(!entry.refCount)
  {
	BGFX_VKAPI(vkDestroySampler)(m_device, sampler, nullptr);
    entry.sampler       = NULL;
