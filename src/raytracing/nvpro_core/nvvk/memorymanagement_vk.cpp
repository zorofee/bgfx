...
class DMAMemoryHandle : public bgfx::MemHandleBase
{
public:
...
private:
  friend class bgfx::DeviceMemoryAllocator;
...
MemHandle DeviceMemoryAllocator::allocMemory(const MemAllocateInfo& allocInfo, VkResult *pResult)
{
  ...
  //     bgfx::DebugUtil(m_dma.getDevice()).setObjectName(memInfo.memory, allocInfo.getDebugName());
  //   }
...
bgfx::AllocationID DeviceMemoryAllocator::getAllocationID(MemHandle memHandle) const
{
  auto dmaHandle = castDMAMemoryHandle(memHandle);
  assert(dmaHandle);
...
bgfx::AllocationID DeviceMemoryAllocator::createID(Allocation& allocation, BlockID block, uint32_t blockOffset, uint32_t blockSize)
{
  // find free slot
...
void DeviceMemoryAllocator::init(VkDevice device, VkPhysicalDevice physicalDevice, VkDeviceSize blockSize, VkDeviceSize maxSize)
{
  ...
  prop2.pNext = &vkProp;
  BGFX_VKAPI(vkGetPhysicalDeviceProperties2)(physicalDevice, &prop2);
...
void DeviceMemoryAllocator::freeAll()
{
  for(const auto& it : m_blocks)
  {
    ...
    if(it.mapped)
    {
	BGFX_VKAPI(vkUnmapMemory)(m_device, it.mem);
    }
	BGFX_VKAPI(vkFreeMemory)(m_device, it.mem, nullptr);
  }
...
void DeviceMemoryAllocator::nvprintReport() const
{
  ...
  LOGI("bgfx::DeviceMemoryAllocator %p\n", this);
  {
...
const bgfx::Allocation& DeviceMemoryAllocator::getAllocation(AllocationID id) const
{
  assert(m_allocations[id.index].id.isEqual(id));
                                                  ...
                                                  const State&                         state)
{
  ...
  if(memReqs.size > m_maxAllocationSize || !bgfx::getMemoryInfo(m_memoryProperties, memReqs, memProps, memInfo, preferDevice))
  {
  ...
  {
    bgfx::DebugUtil(m_device).setObjectName(block.mem, m_debugName);
...
void* DeviceMemoryAllocator::map(AllocationID allocationID, VkResult *pResult)
{
  ...
  if(!block.mapped)
  {
    VkResult result = BGFX_VKAPI(vkMapMemory)(m_device, block.mem, 0, block.allocationSize, 0, (void**)&block.mapped);
    if (pResult)
    {
      *pResult = result;
    }
...
void DeviceMemoryAllocator::unmap(AllocationID allocationID)
{
  ...
  {
    block.mapped = nullptr;
	BGFX_VKAPI(vkUnmapMemory)(m_device, block.mem);
  }
}
                                           ...
                                           VkResult&                result)
{
  ...
  {
    BGFX_VKAPI(vkDestroyImage)(m_device, image, nullptr);
  ...
  result = BGFX_VKAPI(vkBindImageMemory2)(m_device, 1, &bindInfos);
  if(result != VK_SUCCESS)
  {
	BGFX_VKAPI(vkDestroyImage)(m_device, image, nullptr);
    return VK_NULL_HANDLE;
                                                                    ...
                                                                    VkResult&             result)
{
  ...
  result = BGFX_VKAPI(vkCreateAccelerationStructureNV)(m_device, &createInfo, nullptr, &accel);
  if(result != VK_SUCCESS)
  {
    return VK_NULL_HANDLE;
  }
  ...
  BGFX_VKAPI(vkGetAccelerationStructureMemoryRequirementsNV)(m_device, &memInfo, &memReqs);
  ...
  if(allocation.mem == VK_NULL_HANDLE)
  {
	BGFX_VKAPI(vkDestroyAccelerationStructureNV)(m_device, accel, nullptr);
    result = VK_ERROR_OUT_OF_POOL_MEMORY;
    return VK_NULL_HANDLE;
  }
  ...
  result = BGFX_VKAPI(vkBindAccelerationStructureMemoryNV)(m_device, 1, &bind);
  if(result != VK_SUCCESS)
  {
	BGFX_VKAPI(vkDestroyAccelerationStructureNV)(m_device, accel, nullptr);
    free(allocationID);
