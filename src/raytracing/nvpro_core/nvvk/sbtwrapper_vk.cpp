...
// Default setup
//
void bgfx::SBTWrapper::setup(VkDevice                                               device,
                             uint32_t                                               familyIndex,
                             bgfx::ResourceAllocator*                               allocator,
                        ...
                        const std::vector<VkRayTracingPipelineCreateInfoKHR>& librariesInfo /*= {}*/)
{
  ...
  auto result =
      BGFX_VKAPI(vkGetRayTracingShaderGroupHandlesKHR)(m_device, rtPipeline, 0, totalGroupCount, sbtSize, shaderHandleStorage.data());
...
VkDeviceAddress SBTWrapper::getAddress(GroupType t)
{
  ...
  return BGFX_VKAPI(vkGetBufferDeviceAddress)(m_device, &i);  // Aligned on VkMemoryRequirements::alignment which includes shaderGroupBaseAlignment
}
