                           ...
                           const VkImageSubresourceRange& subresourceRange)
{
  ...
  BGFX_VKAPI(vkCmdPipelineBarrier)(cmdbuffer, srcStageMask, destStageMask, 0, 0, nullptr, 0, nullptr, 1, &imageMemoryBarrier);
}
...
void cmdGenerateMipmaps(VkCommandBuffer cmdBuf, VkImage image, VkFormat imageFormat, const VkExtent2D& size, uint32_t levelCount, uint32_t layerCount, VkImageLayout currentLayout)
{
  ...
  barrier.srcQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
  barrier.dstQueueFamilyIndex             = VK_QUEUE_FAMILY_IGNORED;
  BGFX_VKAPI(vkCmdPipelineBarrier)(cmdBuf, pipelineStageForLayout(currentLayout), VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
                       nullptr, 1, &barrier);
  ...
  if(levelCount > 1)
  {
    ...
    barrier.subresourceRange.levelCount   = VK_REMAINING_MIP_LEVELS;
	BGFX_VKAPI(vkCmdPipelineBarrier)(cmdBuf, pipelineStageForLayout(currentLayout), VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr,
                         0, nullptr, 1, &barrier);
  };
  ...
  for(uint32_t i = 1; i < levelCount; i++)
  {
	...
	BGFX_VKAPI(vkCmdBlitImage)(cmdBuf, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
    ...
    // Next
    {
      ...
      barrier.dstAccessMask                 = VK_ACCESS_TRANSFER_READ_BIT;
	  BGFX_VKAPI(vkCmdPipelineBarrier)(cmdBuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
                           nullptr, 1, &barrier);
    }
  ...
  BGFX_VKAPI(vkCmdPipelineBarrier)(cmdBuf, VK_PIPELINE_STAGE_TRANSFER_BIT, pipelineStageForLayout(currentLayout), 0, 0, nullptr, 0,
                       nullptr, 1, &barrier);
}
