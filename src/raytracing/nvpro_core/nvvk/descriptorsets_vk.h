...
inline VkDescriptorSet allocateDescriptorSet(VkDevice device, VkDescriptorPool pool, VkDescriptorSetLayout layout)
{
  ...
  VkDescriptorSet set;
  result = BGFX_VKAPI(vkAllocateDescriptorSets)(device, &allocInfo, &set);
  assert(result == VK_SUCCESS);
  return set;
}
                                   ...
                                   std::vector<VkDescriptorSet>& sets)
{
  ...
  result = BGFX_VKAPI(vkAllocateDescriptorSets)(device, &allocInfo, sets.data());
  assert(result == VK_SUCCESS);
}
#ifdef VULKAN_HPP
                                                                      ...
                                                                      VkPipelineLayoutCreateFlags flags /*= 0*/)
{
  ...
  result = BGFX_VKAPI(vkCreatePipelineLayout)(device, &layoutCreateInfo, nullptr, &m_pipelayouts[pipe]);
  assert(result == VK_SUCCESS);
  return m_pipelayouts[pipe];
}
...
void TDescriptorSetContainer<SETS, PIPES>::deinitLayouts()
{
  ...
  for(int p = 0; p < PIPES; p++)
  {
    if(m_pipelayouts[p])
    {
	  BGFX_VKAPI(vkDestroyPipelineLayout)(device, m_pipelayouts[p], nullptr);
