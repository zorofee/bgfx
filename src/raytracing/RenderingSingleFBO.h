...
namespace bgfx
{
	...
	public:
		...
		void createCommandBuffers(VkExtent2D swapChainExtent) {
			...
			if (BGFX_VKAPI(vkAllocateCommandBuffers)(device, &allocInfo, &commandBuffer) != VK_SUCCESS) {
...

			{
				...
				if (BGFX_VKAPI(vkBeginCommandBuffer)(commandBuffer, &beginInfo) != VK_SUCCESS) {
				...
				BGFX_VKAPI(vkCmdBeginRenderPass)(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

				BGFX_VKAPI(vkCmdBindPipeline)(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);

				BGFX_VKAPI(vkCmdDraw)(commandBuffer, 3, 1, 0, 0);

				BGFX_VKAPI(vkCmdEndRenderPass)(commandBuffer);

				if (BGFX_VKAPI(vkEndCommandBuffer)(commandBuffer) != VK_SUCCESS) {
